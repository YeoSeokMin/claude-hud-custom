...
export function renderIdentityLine(ctx: RenderContext): string {
  ...
  // Token usage display (e.g., 137K/200K)
  ...
  if (display?.showModel !== false && display?.showContextBar !== false) {
    parts.push(`${cyan(`[${modelDisplay}]`)} ${bar} ${getContextColor(percent)}${percent}% ${dim(tokenDisplay)}${RESET}`);
  } else if (display?.showModel !== false) {
    parts.push(`${cyan(`[${modelDisplay}]`)} ${getContextColor(percent)}${percent}% ${dim(tokenDisplay)}${RESET}`);
  } else if (display?.showContextBar !== false) {
    parts.push(`${bar} ${getContextColor(percent)}${percent}% ${dim(tokenDisplay)}${RESET}`);
  } else {
    parts.push(`${getContextColor(percent)}${percent}% ${dim(tokenDisplay)}${RESET}`);
  }
...
function renderInlineUsage(ctx: RenderContext): string | null {
  ...
  if (isLimitReached(ctx.usageData)) {
    ...
    return red(`⚠ 한도${resetTime ? ` (${resetTime})` : ''}`);
  }
